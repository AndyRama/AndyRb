...
const HostBaseCard = ({ image, text, button, variant }) => {
  ...
  return (
    <div className={wrapperClassList.join(' ')}>
      <picture className={illustrationClassList.join(' ')}>
        <source
          srcSet={`${image?.url_min2080_1x} 1x, ${image?.url_min2080_2x} 2x,`}
          media="(min-width: 2080px)"
        />
        <source
          srcSet={`${image?.url_min1440_1x} 1x, ${image?.url_min1440_2x} 2x,`}
          media="(min-width: 1440px)"
        />
        <source
          srcSet={`${image?.url_min1128_1x} 1x, ${image?.url_min1128_2x} 2x,`}
          media="(min-width: 1128px)"
        />
        <source
          srcSet={`${image?.url_min744_1x} 1x, ${image?.url_min744_2x} 2x,`}
          media="(min-width: 744px)"
        />
        <source
          srcSet={`${image?.url_min375_1x} 1x, ${image?.url_min375_2x} 2x,`}
          media="(min-width: 375px)"
        />
        <source
          srcSet={`${image?.url_nomin_1x} 1x, ${image?.url_nomin_2x} 2x,`}
        />
        <img src={image?.url_default} alt={`${image?.alt}`} />
      </picture>
      <div className={textClassList.join(' ')}>{text}</div>
      <a className={buttonClassList.join(' ')} href={button?.target}>
