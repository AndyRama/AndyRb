...
const getPlaceType = ([placeTypeId, precisePlaceTypeId]) => {
  const placeType = placeTypeEnum.find(
    (placeType) => placeType.id === placeTypeId
  )
  const precisePlaceType = placeType.placeKind.find(
    (precisePlaceType) => precisePlaceType.id === precisePlaceTypeId
  )
  return precisePlaceType.name
}
const getHousingType = (id) => {
  console.log(id)
  const housingType = housingTypeEnum.find(
    (housingType) => housingType.id === id
  )
